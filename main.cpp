...
#pragma region LoadTexture
	...
	int playerTH = Novice::LoadTexture("./RS/Player/Player1-Sheet.png");//(234x66)
...
#pragma endregion
	vector2_t p0 = { player.pos.x,player.pos.y };
...
#pragma endregion
	...
	while (Novice::ProcessMessage() == 0) {
		...
		for (int i = 0; i < enemies.size(); i++) {
			if (enemies[i].isAlive) {
                if (pow((player.pos.x - enemies[i].pos.x), 2) + pow((player.pos.y - enemies[i].pos.y), 2) <= pow((player.hitboxRadius+55 + enemies[i].hitboxRadius), 2)) {
...
#pragma endregion
		if (Novice::IsTriggerMouse(0)&&isMislShot==false&& mislRem!=0) {
            ...
            p0 = { player.pos.x,player.pos.y+10.0f };
			missile.isShot = true;
			isMislShot = true;
		...
		if (upgradeItem_1.isSpawn) {
			...
			if (pow((player.pos.x - upgradeItem_1.pos.x), 2) + pow((player.pos.y - upgradeItem_1.pos.y), 2) <= pow((player.hitboxRadius + 80 + upgradeItem_1.hitboxRadius), 2)) {
				upgradeItem_1.isSpawn = false;
		...
		if (upgradeItem_2.isSpawn) {
			...
			if (pow((player.pos.x - upgradeItem_2.pos.x), 2) + pow((player.pos.y - upgradeItem_2.pos.y), 2) <= pow((player.hitboxRadius + 80 + upgradeItem_2.hitboxRadius), 2)) {
				upgradeItem_2.isSpawn = false;
				if (player.weaponLV == 1) {
					player.weaponLV = 2;
				}
...
#pragma endregion
		...
		player.pos = MovePlayer(keys, player,tHRHEAT_HUD);
...
#pragma endregion
		...
		if (player.isOC && wPNOC_t.timer < 300) {
			wPNOC_t.timer++;
			player.weaponRate = 5;
            if (player.weaponLV == 1) {
				oC_1HUD.isFlash = false;
		...
		//=======================Elite====================================================
		if (timer==2000) {
			elite.target = { player.pos.x,player.pos.y };
		...
		for (auto& eli : elites) {
			...
			if (eli.pos.x > 1955 || eli.pos.x < -35 || eli.pos.y>1820 || eli.pos.y < -745) {
				eli.moveCase = 1;
				eli.pos.x = player.pos.x;
				eli.pos.y = 1815.0f;
			}
			if (eli.moveCase == 1) {
				eli.speed.y = -3.0f;
				eli.angle=3.15f;
				if (eli.pos.y > player.pos.y+150.0f) {
                    if (eli.pos.x - player.pos.x > 4) {
				    	eli.speed.x = -2.0f;
				    }
				    else if (eli.pos.x - player.pos.x < -4) {
				    	eli.speed.x = 2.0f;
				    }
...
#pragma region Radar
		for (auto& rEHUD : radarEnemyHUDs) {
			...
			for (auto& ene : enemies) {
				if (ene.isAlive) {
					rEHUD.pos = { radarOriginal.x + (ene.pos.x - player.pos.x) / 5,radarOriginal.y + (ene.pos.y - player.pos.y) / 5 };
...
#pragma endregion
		...
		if (player.isAlive) {
			if (playerFlameAniNum <= 2) {
				Novice::DrawSpriteRect(int(player.pos.x - 18), int(player.pos.y + 28), playerFlame.currentFrame * 36, 0, 36, 30, playerFlameAni[playerFlameAniNum], 1.0f / 4.0f, 1, 0, WHITE);
			}
			else {
				Novice::DrawSpriteRect(int(player.pos.x - 16), int(player.pos.y + 28), playerFlame.currentFrame * 30, 0, 30, 30, playerFlameAni[playerFlameAniNum], 1.0f / 4.0f, 1, 0, WHITE);
			}
			Novice::DrawSpriteRect((int)player.pos.x - 78 / 2, (int)player.pos.y - 66 / 2, (int)player.drawPosX, 0, 78, 66, playerTH, 1.0f / 3.0f, 1, 0, WHITE);
		}
...
#pragma region ScreenPrintf
		...
		Novice::ScreenPrintf(359, 125, "Player HP %d", player.hp);
